...
│   └── tests/e2e/docker-compose.e2e.yml     # Testing (TDD)
│
...
- **`tests/`:** Docker Compose isolado para testes de integração. Execute `docker compose -f tests/e2e/docker-compose.e2e.yml up` para rodar a suite completa.
...
| `tests/e2e/docker-compose.e2e.yml` | Testing | **docker/** |
