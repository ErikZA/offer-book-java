...
* **Índice de Idempotência (`idx_history_eventId`) — AT-05.2:** Cada entrada do array `history` carrega um `eventId` único. O consumer de projeção aplica um filtro atômico `{"history.eventId": {$ne: eventId}}` em cada `$push` para garantir que o mesmo evento nunca seja gravado duas vezes. Sem índice, esse filtro percorreria o array inteiro (O(n) por operação). O índice multikey `{"history.eventId": 1}` reduz essa busca para O(log n), mantendo a performance mesmo com históricos longos e alta concorrência.
