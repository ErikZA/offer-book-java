-- ==============================================================================
-- V7: Event Store Imutável (AT-14)
--
-- Propósito: manter um log append-only de todos os eventos de domínio emitidos
-- pelo order-service. Complementar ao Transactional Outbox — enquanto a tabela
-- tb_order_outbox serve como relay temporário para o broker, o Event Store é
-- o registro permanente e imutável para:
--   - Replay completo de eventos
--   - Audit trail para compliance regulatório
--   - Reconstrução de estado de qualquer agregado em qualquer ponto no tempo
--
...
CREATE TABLE IF NOT EXISTS tb_event_store (
    ...
    -- Tipo do evento (ex: "OrderReceivedEvent", "FundsReservedEvent")
...
COMMENT ON TABLE  tb_event_store IS
    ...
    'de domínio emitidos pelo order-service. Protegido por trigger contra UPDATE/DELETE.';
